...
=== " only_r18 = `true` / `false` "
    - During startup (if a [REDIS_URL](http://127.0.0.1:8900/deploy/#variable-template) found) a scrapper scraps all the pages found in r18.com's actress section and saves them in redis database as _key_ : _value_ pair (`name` : `image_url`).
    - Searching in redis-database is lot faster than searching and scraping a webpage for actress informantion only if there is lot more query to search (ex. many actresses in same movie.
