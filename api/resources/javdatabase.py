...
async def parse_details(
    tree: html.HtmlElement, only_r18: bool
) -> (dict[str, str] | None):
    """Parse movie details."""
    movie_dictionary = {}
    if not bool(re.search("Page Not Found", tree.find("head/title").text)):
        ...
        movie_dictionary["title"] = tree.find('.//td[@class="tablevalue"]').text.strip()
...
async def parse_additional_details(tree: html.HtmlElement) -> (dict[str, str] | None):
    ...
    for parent in tree.xpath("//tr"):
        try:
            ...
            details["director"] = (
                value if bool(re.search("Director", key)) and len(value) > 0 else None
            )
...
async def parse_actress_details(
    tree: html.HtmlElement, only_r18: bool
) -> (dict[str, str] | None):
